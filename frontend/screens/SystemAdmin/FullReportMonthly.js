...
const FullReportMonthly = ({ route }) => {
  ...
  const fetchOrderHistoryForUser = async (userId) => {
    try {
      ...
      const currentMonthYear = getCurrentMonthFormatted();
      console.log("Current Month/Year:", currentMonthYear); // Log the current month/year

      const filteredOrders = data.filter((order) => {
        const dateParts = order.dateToDeliver.split("/");
        const orderMonthYear = `${String(parseInt(dateParts[1])).padStart(
          2,
          "0"
        )}/${dateParts[2]}`; // Adjust to MM/YYYY format
        return (
          order.submittedById === userId && orderMonthYear === currentMonthYear
        );
      });

      console.log("Filtered Orders:", filteredOrders); // Log the filtered data
...
export default FullReportMonthly;
